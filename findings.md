...
- 第二轮精修中，标题与中文展示字体已切换为更强调出版感的组合。
- 首页背景已从带明显光晕的渐变改为更统一的暖白底。
- 文章页已移除底部外链分享区，TOC 在无目录时不再渲染。
- `search`、`tags`、`categories`、`archives` 需要单独模板覆盖，否则只会继承部分颜色而不会继承统一浏览结构。
- 首页与 post 页背景要完全一致，不能只改卡片和控件底色，还要覆盖 `html`、`body`、`body.list`、`body#top` 和共享容器层。
